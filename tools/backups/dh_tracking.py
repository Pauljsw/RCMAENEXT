...
def pc_cb(pcslam_msg, odom_msg, raw_pc_msg):
    ...
    if time.time() % 1 < 0.1:
        ...
        track_magenta_boxes(predictions)


def track_magenta_boxes(predictions):
    global previous_positions
    new_positions = []
    marker_array = MarkerArray()  # Marker array to hold all line markers
    marker_id = 0  # Unique ID for each line marker
    
    for pred in predictions:
        pred_boxes = pred['pred_boxes']
        for box in pred_boxes:
            # Store center positions of each magenta box (x, y, z)
            new_positions.append((box[0].cpu().numpy(), box[1].cpu().numpy(), box[2].cpu().numpy()))

    # Update previous positions for the next frame
    previous_positions = new_positions

    # Publish the tracking lines to RViz
    tracking_line_pub.publish(marker_array)
...
tracking_line_pub = rospy.Publisher('/tracking_lines', MarkerArray, queue_size=10)
