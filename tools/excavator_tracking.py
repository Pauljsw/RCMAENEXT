    ...
    for pred in predictions:
        ...
        for box, score, label in zip(pred_boxes, pred_scores, pred_labels):
            if score > 0.5:
                ...
                x_slam = round(np.cos(theta) * center_x - np.sin(theta) * center_y + x, 1)
                y_slam = round(np.sin(theta) * center_x + np.cos(theta) * center_y + y, 1)
                center_z = round(center_z, 1)
...
# 1106 
latest_pcslam_msg = None
latest_odom_msg = None
latest_raw_pc_msg = None
def pcslam_callback(msg):
    global latest_pcslam_msg
    latest_pcslam_msg = msg
def odometry_callback(msg):
    global latest_odom_msg
    latest_odom_msg = msg
def raw_pc_callback(msg):
    global latest_raw_pc_msg
    latest_raw_pc_msg = msg
def timer_callback(event):
    if latest_pcslam_msg and latest_odom_msg and latest_raw_pc_msg:
        pc_cb(latest_pcslam_msg, latest_odom_msg, latest_raw_pc_msg)
...
pcslam_sub = rospy.Subscriber('/lio_sam/mapping/cloud_output', PointCloud2, pcslam_callback)
odo_sub = rospy.Subscriber('/lio_sam/mapping/odometry', Odometry, odometry_callback)
pcraw_sub = rospy.Subscriber("/lio_sam/deskew/cloud_deskewed", PointCloud2, raw_pc_callback)

#publishers
marker_pub = rospy.Publisher('/bounding_boxes', MarkerArray, queue_size=10)


# Set up a timer to call pc_cb at 1 Hz
