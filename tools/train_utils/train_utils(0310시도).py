        ...
        for cur_epoch in tbar:
            ...
            with torch.no_grad():
                ...
                for val_batch_idx, val_batch_data in enumerate(val_loader):
                    print(f"🚀 Validation: Processing batch {val_batch_idx}")
                    val_batch_data["target_dicts"] = model.generate_target_dicts(val_batch_data)
                    result = model_func(model, val_batch_data)
                    
                    # result가 loss, pred_dict, recall_dict, tb_dict, disp_dict을 포함해야 함
                    if isinstance(result, tuple) and len(result) == 5:
                        loss, pred_dict, recall_dict, tb_dict, disp_dict = result
                    else:
                        print("❌ Error: Unexpected result format from model_func during validation")
                        continue
            ...
            if use_logger_to_record:
                logger.info('Validation loss after epoch %d: %.4f' % (cur_epoch + 1, val_loss))
