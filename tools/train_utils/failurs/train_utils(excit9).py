...
def train_one_epoch(model, optimizer, train_loader, model_func, lr_scheduler, accumulated_iter, optim_cfg,
                    ...
                    total_epochs=None, ckpt_save_dir=None, ckpt_save_time_interval=300, show_gpu_stat=False, use_amp=False, k_folds=5):
    ...
    for fold, (train_ids, val_ids) in enumerate(kfold.split(all_indices)):
        train_subsampler = torch.utils.data.Subset(all_data, train_ids)
        val_subsampler = torch.utils.data.Subset(all_data, val_ids)

        train_loader_fold = torch.utils.data.DataLoader(train_subsampler, batch_size=train_loader.batch_size, shuffle=True, num_workers=train_loader.num_workers)
        val_loader_fold = torch.utils.data.DataLoader(val_subsampler, batch_size=train_loader.batch_size, shuffle=False, num_workers=train_loader.num_workers)
        
        dataloader_iter = iter(train_loader_fold)
        ckpt_save_cnt = 1
        ...
        if rank == 0:
            pbar = tqdm.tqdm(total=total_it_each_epoch, leave=leave_pbar, desc=f'train_fold_{fold}', dynamic_ncols=True)
    ...
    avg_val_loss = sum(val_losses) / k_folds
    if rank == 0:
        print(f'Average Validation Loss: {avg_val_loss:.4f}')
    return accumulated_iter, avg_val_loss
...
def train_model(model, optimizer, train_loader, model_func, lr_scheduler, optim_cfg,
                ...
                k_folds=5):  # Added k_folds parameter
    ...
    with tqdm.trange(start_epoch, total_epochs, desc='epochs', dynamic_ncols=True, leave=(rank == 0)) as tbar:
        ...
        for cur_epoch in tbar:
            ...
            # train one epoch with k-fold cross-validation
            if lr_warmup_scheduler is not None and cur_epoch < optim_cfg.WARMUP_EPOCH:
                cur_scheduler = lr_warmup_scheduler
            else:
                cur_scheduler = lr_scheduler
            
            accumulated_iter, avg_val_loss = train_one_epoch(
                ...
                use_amp=use_amp,
                k_folds=k_folds  # Pass k_folds to train_one_epoch
            )
            ...
            if trained_epoch % ckpt_save_interval == 0 and rank == 0:
                ...
                logger.info(f'Save checkpoint to {ckpt_name}')
            
            # Optionally, log or save validation loss
            if rank == 0:
                logger.info(f'Epoch {cur_epoch + 1}/{total_epochs}, Average Validation Loss: {avg_val_loss:.4f}')
                if tb_log is not None:
                    tb_log.add_scalar('val/avg_loss', avg_val_loss, cur_epoch)
