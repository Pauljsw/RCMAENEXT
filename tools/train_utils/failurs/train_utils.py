...
def train_model(model, optimizer, dataset, model_func, lr_scheduler, optim_cfg,
                start_epoch, total_epochs, start_iter, rank, tb_log, ckpt_save_dir, train_sampler=None,
                lr_warmup_scheduler=None, ckpt_save_interval=1, max_ckpt_save_num=50,
                merge_all_iters_to_one_epoch=False, use_amp=False,
                use_logger_to_record=False, logger=None, logger_iter_interval=None, ckpt_save_time_interval=None, show_gpu_stat=False, k_folds=5):
    ...
    for fold in range(k_folds):
        ...
        with tqdm.trange(start_epoch, total_epochs, desc='epochs', dynamic_ncols=True, leave=(rank == 0)) as tbar:
            ...
            for cur_epoch in tbar:
                ...
                # validate one epoch
                validate_one_epoch(
                    model, val_loader, model_func, rank, tb_log, accumulated_iter, cur_epoch, use_logger_to_record, logger
                )
...
def validate_one_epoch(model, val_loader, model_func, rank, tb_log, accumulated_iter, cur_epoch, use_logger_to_record, logger):
    model.eval()
    with torch.no_grad():
        for batch in val_loader:
            ...
            if use_logger_to_record:
                logger.info(f"Validation loss at epoch {cur_epoch}: {loss.item()}")
