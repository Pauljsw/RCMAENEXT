...
class CustomDataset(DatasetTemplate):
    def __init__(self, dataset_cfg, class_names, training=True, root_path=None, logger=None):
        ...
        split_dir_path = os.path.join(self.root_path, 'Split', (self.split + '.txt'))
        if os.path.exists(split_dir_path):
            with open(split_dir_path, 'r') as f:
                # Read the first line which should be the directory path
                directory_path = f.readline().strip()
                # Build the sample_id_list by listing all files in the directory_path
                self.sample_id_list = [os.path.splitext(os.path.basename(x))[0] for x in glob.glob(os.path.join(directory_path, '*.pcd'))]
        else:
    ...
    def get_label(self, idx):
        label_file = self.root_path / 'labels' / f'{idx}.json'
        ...
        for obj in data['objects']:
            ...
            x, y, z = obj['centroid'].values()
            dx, dy, dz = obj['dimensions']['length'], obj['dimensions']['width'], obj['dimensions']['height']
            # Assuming the 'z' rotation is the heading angle, and converting to radians if necessary
            heading_angle = np.radians(obj['rotations']['z'])
            name = obj['name']
            # Construct the box information and append
...
def create_custom_infos(dataset_cfg, class_names, data_path, save_path, workers=4):
    dataset = CustomDataset(
        dataset_cfg=dataset_cfg, class_names=['Worker', 'Car', 'Equipment_Transportation', 'Equipment_Operational'], root_path=data_path,
