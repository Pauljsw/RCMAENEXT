...
def clean_data(gt_anno, dt_anno, current_class, difficulty):
    ...
    #MIN_HEIGHT = [25, 25, 25]
    #MAX_OCCLUSION = [2, 2, 2]
    #MAX_TRUNCATION = [0.5, 0.5, 0.5]
    MIN_HEIGHT = [0, 0, 0]
    MAX_OCCLUSION = [100, 100, 100]
    MAX_TRUNCATION = [100, 100, 100]
...
def _prepare_data(gt_annos, dt_annos, current_class, difficulty):
    ...
    for i in range(len(gt_annos)):
        ...
        #0311
        #print(f"Class {current_class}: num_valid_gt (per scene) = {num_valid_gt}")
        
        ignored_gts.append(np.array(ignored_gt, dtype=np.int64))
        ignored_dets.append(np.array(ignored_det, dtype=np.int64))
        
        #print(f"Class {current_class}: ignored_gt count = {sum(1 for x in ignored_gt if x != 0)}")
    ...
    #print(f"Class {current_class}: ignored_gts count = {sum(len(x) for x in ignored_gts)}")
...
def eval_class(gt_annos,
    ...
    for m, current_class in enumerate(current_classes):
        for l, difficulty in enumerate(difficultys):
            ...
            #0311 
            print(f"Class {current_class}: Total Valid GTs = {total_num_valid_gt}")
            #print(f"Class {current_class}: min_overlaps = {min_overlaps[:, metric, m]}")
            ...
            for k, min_overlap in enumerate(min_overlaps[:, metric, m]):
                ...
                for i in range(len(gt_annos)):
                    ...
                    #0311
                    #print(f"Class {current_class}: TP = {tp}, FP = {fp}, FN = {fn}")
                ...
                #0311
                #print(f"Class {current_class}: PR Thresholds = {thresholds}")
...
def get_official_eval_result(gt_annos, dt_annos, current_classes, PR_detail_dict=None):
    ...
    #overlap_0_5 = np.array([[0.7, 0.5, 0.5, 0.7, 0.5, 0.5], [0.5, 0.25, 0.25, 0.5, 0.25, 0.5], [0.5, 0.25, 0.25, 0.5, 0.25, 0.5]])
    
    #0527
    overlap_high = np.array([[0.7, 0.7, 0.7, 0.7, 0.7, 0.7], 
                              [0.5, 0.5, 0.5, 0.5, 0.5,0.5], 
                              [0.5, 0.5, 0.5, 0.5, 0.5, 0.5]])
    overlap_low = np.array([[0.5, 0.5, 0.5, 0.5, 0.5, 0.5], 
                            [0.25, 0.25, 0.25, 0.25, 0.25, 0.25],
                            [0.25, 0.25, 0.25, 0.25, 0.25, 0.25]])
    min_overlaps = np.stack([overlap_high, overlap_low], axis=0)
    ...
    for j, curcls in enumerate(current_classes):
        ...
        if compute_aos:
            result += print_str((f"aos  AP:{mAPaos[j, 0]:.2f}, "
                                 f"{mAPaos[j, 1]:.2f}, "
                                 f"{mAPaos[j, 2]:.2f}"))
        print(f"{class_to_name[curcls]} Precision: {mAPbbox[j, :]}")
        print(f"{class_to_name[curcls]} Recall: {mAPbev[j, :]}")                             
    return result
